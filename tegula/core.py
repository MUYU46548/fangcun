...
# 兼容包结构（core.py 在 tegula/ 子目录下）：向上查父目录找 legacy 标记文件
_legacy_reg = os.path.join(APP_DIR, "registry.yaml")
_legacy_task = os.path.join(APP_DIR, "task-data")
if not os.path.exists(_legacy_reg) and not os.path.isdir(_legacy_task):
    # 当前目录没有 legacy 标记，尝试父目录（包结构下 core.py 在 tegula/ 子目录）
    _parent_dir = os.path.dirname(APP_DIR)
    if _parent_dir and _parent_dir != APP_DIR:
        _parent_reg = os.path.join(_parent_dir, "registry.yaml")
        _parent_task = os.path.join(_parent_dir, "task-data")
        if os.path.exists(_parent_reg) or os.path.isdir(_parent_task):
            APP_DIR = _parent_dir
            _legacy_reg = _parent_reg
            _legacy_task = _parent_task
...
MCP_TOOLS = [
    ...
    {"name": "get_task", "description": "获取单个任务的完整信息（含方案进度）",
     "inputSchema": {"type": "object", "properties": {"id": {"type": "string", "description": "任务 id"}}}},
    {"name": "list_projects", "description": "列出所有已注册项目",
     "inputSchema": {"type": "object", "properties": {}}},
    {"name": "get_project_status", "description": "获取项目状态（git 活动 + 任务健康度 + 建议）",
     "inputSchema": {"type": "object", "properties": {"id": {"type": "string", "description": "项目 id"}}}},
    {"name": "search_tasks", "description": "全文搜索任务（标题 + 正文）",
     "inputSchema": {"type": "object", "properties": {
         "q": {"type": "string", "description": "搜索关键词"}}}},
    {"name": "get_roadmap", "description": "获取路线图聚合视图（按项目/批次）",
     "inputSchema": {"type": "object", "properties": {
         "project": {"type": "string", "description": "项目 id（可选，不传返回全部）"}}}},
    {"name": "get_roadmap_full", "description": "获取完整路线图（含里程碑 + 并行建议 + 趋势）",
     "inputSchema": {"type": "object", "properties": {}}},
    {"name": "plan_list", "description": "列出所有决策计划",
     "inputSchema": {"type": "object", "properties": {}}},
    {"name": "plan_get", "description": "获取单个决策计划",
     "inputSchema": {"type": "object", "properties": {"id": {"type": "string", "description": "计划 id"}}}},
    {"name": "plan_pending_decisions", "description": "列出所有待决策项",
     "inputSchema": {"type": "object", "properties": {}}},
    {"name": "gate_open", "description": "开启验收门槛",
     "inputSchema": {"type": "object", "properties": {"id": {"type": "string", "description": "任务 id"}}}},
    {"name": "gate_check", "description": "检查验收门槛状态",
     "inputSchema": {"type": "object", "properties": {"id": {"type": "string", "description": "任务 id"}}}},
    {"name": "gate_list", "description": "列出所有开启的验收门槛",
     "inputSchema": {"type": "object", "properties": {}}},
    {"name": "gate_close", "description": "关闭验收门槛",
     "inputSchema": {"type": "object", "properties": {"id": {"type": "string", "description": "任务 id"}}}},
]
