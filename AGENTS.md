...
> frontmatter 序列化为**白名单 + 透传**：受管字段（id/标题/项目/状态/批次/截止/优先级/来源/指派/验收/资源）由方寸管理；其余未知字段保留键序原样回写。外部 agent 加字段不会在下次保存时丢失。
...
### 并发写防护（乐观锁）
- GUI 的编辑/勾选/拖拽均携带 `expected_mtime`；服务端发现磁盘 mtime 不符即拒绝并要求刷新，后写者不再静默覆盖先写者。
